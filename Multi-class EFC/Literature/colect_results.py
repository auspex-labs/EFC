...
def single_attack(folder, attacks):
    ...
    with open(f"{folder}/1by1_aurocs.txt", mode="w+") as file:
        for attack in attacks:
            y_test = pd.read_csv(f"{folder}/1by1_sets/{attack}.csv", header=None).values[:, -1]
            y_score = np.load(f"{folder}/Results/1by1/EFC_{attack}.npy", allow_pickle=True)[1]
            probs_rvs = [x for x in MinMaxScaler().fit_transform(y_score.reshape(-1, 1))]
            ...
            precision_rvs, recall_rvs, _ = precision_recall_curve(y_test, probs_rvs, pos_label=1)
