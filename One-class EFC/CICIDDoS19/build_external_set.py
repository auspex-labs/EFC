...
for i in range(1, 11):
    ...
    external_test = [
        x[1:-1]
        for x in normals[int(len(normals) * 0.8) : :]
        + DrDoS_DNS[int(len(DrDoS_DNS) * 0.8) : :]
        + DrDoS_LDAP[int(len(DrDoS_LDAP) * 0.8) : :]
    ]
    external_test += [
        x[1:-1]
        for x in DrDoS_MSSQL[int(len(DrDoS_MSSQL) * 0.8) : :]
        + DrDoS_NetBIOS[int(len(DrDoS_NetBIOS) * 0.8) : :]
        + DrDoS_NTP[int(len(DrDoS_NTP) * 0.8) : :]
        + DrDoS_SNMP[int(len(DrDoS_SNMP) * 0.8) : :]
        + DrDoS_SSDP[int(len(DrDoS_SSDP) * 0.8) : :]
    ]
    external_test += [
        x[1:-1]
        for x in DrDoS_UDP[int(len(DrDoS_UDP) * 0.8) : :]
        + Syn[int(len(Syn) * 0.8) : :]
        + TFTP[int(len(TFTP) * 0.8) : :]
        + LDAP[int(len(LDAP) * 0.8) : :]
        + NetBIOS[int(len(NetBIOS) * 0.8) : :]
        + MSSQL[int(len(MSSQL) * 0.8) : :]
    ]
    external_test += [
        x[1:-1] for x in Portmap[int(len(Portmap) * 0.8) : :] + UDP[int(len(UDP) * 0.8) : :] + UDP_lag[int(len(UDP_lag) * 0.8) : :]
    ]
    ...
    external_test_labels += [
        x[-1] for x in Portmap[int(len(Portmap) * 0.8) : :] + UDP[int(len(UDP) * 0.8) : :] + UDP_lag[int(len(UDP_lag) * 0.8) : :]
    ]

    np.save(f"External_test/Discretized/Exp{i}/external_test.npy", np.array(external_test))
    np.save(f"External_test/Discretized/Exp{i}/external_test_labels.npy", np.array(external_test_labels))

    # Non_discretized test
    external_test = [
        x[1:-1]
        for x in normals_pre[int(len(normals) * 0.8) : :]
        + DrDoS_DNS_pre[int(len(DrDoS_DNS) * 0.8) : :]
        + DrDoS_LDAP_pre[int(len(DrDoS_LDAP) * 0.8) : :]
    ]
    external_test += [
        x[1:-1]
        for x in DrDoS_MSSQL_pre[int(len(DrDoS_MSSQL) * 0.8) : :]
        + DrDoS_NetBIOS_pre[int(len(DrDoS_NetBIOS) * 0.8) : :]
        + DrDoS_NTP_pre[int(len(DrDoS_NTP) * 0.8) : :]
        + DrDoS_SNMP_pre[int(len(DrDoS_SNMP) * 0.8) : :]
        + DrDoS_SSDP_pre[int(len(DrDoS_SSDP) * 0.8) : :]
    ]
    external_test += [
        x[1:-1]
        for x in DrDoS_UDP_pre[int(len(DrDoS_UDP) * 0.8) : :]
        + Syn_pre[int(len(Syn) * 0.8) : :]
        + TFTP_pre[int(len(TFTP) * 0.8) : :]
        + LDAP_pre[int(len(LDAP) * 0.8) : :]
        + NetBIOS_pre[int(len(NetBIOS) * 0.8) : :]
        + MSSQL_pre[int(len(MSSQL) * 0.8) : :]
    ]
    external_test += [
        x[1:-1]
        for x in Portmap_pre[int(len(Portmap) * 0.8) : :]
        + UDP_pre[int(len(UDP) * 0.8) : :]
        + UDP_lag_pre[int(len(UDP_lag) * 0.8) : :]
    ]
    ...
    np.save(f"External_test/Non_discretized/Exp{i}/external_test.npy", np.array(external_test))
