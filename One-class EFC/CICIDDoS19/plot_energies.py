...
for set in range(1, 11):
    energies = np.load(f"Data/Discretized/Exp{set}/energies_internal.npy", allow_pickle=True)
    ...
    cutoff += np.load(f"Data/Discretized/Exp{set}/cutoff.npy", allow_pickle=True)
cutoff /= 10
...
cutoff = 0
for set in range(1, 11):
    energies = np.load(f"Data/Discretized/Exp{set}/energies_external.npy", allow_pickle=True)
    data_normal = np.concatenate((data_normal, energies[: len(energies) // 2]), axis=0)
    data_atk = np.concatenate((data_atk, energies[len(energies) // 2 : :]), axis=0)
    cutoff += np.load(f"Data/Discretized/Exp{set}/cutoff.npy", allow_pickle=True)
cutoff /= 10
