...
LABELS = [
    ...
    ("attackType", 0, 0),
    ("attackID", 0, 0),
    ("attackDescription", 0, 0),
]  # name, used, must_digitize
N_TRAIN = 10000
N_TEST = 10000
Q = 32  # alphabet size
...
normal_all = []
if TRAFFIC == "ext":
    with open(PATH_EXT + "new-unknown-data-all.csv") as fl:
        raw = [x for x in fl.read().split("\n")[:-1]]
        normal_all += [x.split(",")[:-1] for x in raw]
else:
    with open(PATH + "new-normal-data-all.csv") as fl:
        raw = [x for x in fl.read().split("\n")[:-1]]
        normal_all += [x.split(",")[:-1] for x in raw]
...
if TRAFFIC != "ext":
    ...
    with open(PATH_EXT + atk_file) as fl:
        raw = [x for x in fl.read().split("\n")[:-1] if len(x.split(",")) > 1]
...
if TRAINING_4_DNN:
    ...
    with open(OUT_PATH + f"non-discretized/{REP}_test_cidds_{TRAFFIC}.csv", "w") as fl:
...
np.save(OUT_PATH + f"discretized/{REP}_test_{TRAFFIC}", np.array(list(all_normal) + list(all_atk)))
np.save(OUT_PATH + f"discretized/{REP}_labels_{TRAFFIC}", labels)
