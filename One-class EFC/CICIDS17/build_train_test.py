...
for i in range(1, 11):
    ...
    train = [
        x[1:-1] for x in normals[: int(len(normals) * 0.8)] + dos[: int(len(dos) * 0.8)] + portscan[: int(len(portscan) * 0.8)]
    ]
    train += [
        x[1:-1]
        for x in bot[: int(len(bot) * 0.8)]
    ...
    train = [
        x[1:-1]
        for x in normals_pre[: int(len(normals) * 0.8)] + dos_pre[: int(len(dos) * 0.8)] + portscan_pre[: int(len(portscan) * 0.8)]
    ]
    train += [
        x[1:-1]
        for x in bot_pre[: int(len(bot) * 0.8)]
