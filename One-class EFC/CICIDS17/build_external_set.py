...
for i in range(1, 11):
    ...
    external_test = [
        x[1:-1]
        for x in normals[int(len(normals) * 0.8) : :] + dos[int(len(dos) * 0.8) : :] + portscan[int(len(portscan) * 0.8) : :]
    ]
    external_test += [
        x[1:-1]
        for x in bot[int(len(bot) * 0.8) : :]
        + inf[int(len(inf) * 0.8) : :]
        + bruteforce[int(len(bruteforce) * 0.8) : :]
        + sqlinj[int(len(sqlinj) * 0.8) : :]
        + xss[int(len(xss) * 0.8) : :]
    ]
    external_test += [
        x[1:-1]
        for x in ftp[int(len(ftp) * 0.8) : :]
        + ssh[int(len(ssh) * 0.8) : :]
        + hulk[int(len(hulk) * 0.8) : :]
        + goldeneye[int(len(goldeneye) * 0.8) : :]
        + slowloris[int(len(slowloris) * 0.8) : :]
        + slowhttp[int(len(slowhttp) * 0.8) : :]
    ]

    external_test_labels = [
        x[-1] for x in normals[int(len(normals) * 0.8) : :] + dos[int(len(dos) * 0.8) : :] + portscan[int(len(portscan) * 0.8) : :]
    ]
    external_test_labels += [
        x[-1]
        for x in bot[int(len(bot) * 0.8) : :]
    ...
    np.save(f"External_test/Discretized/Exp{i}/external_test.npy", np.array(external_test, dtype=int))
    np.save(f"External_test/Discretized/Exp{i}/external_test_labels.npy", np.array(external_test_labels, dtype=int))

    external_test = [
        x[1:-1]
        for x in normals_pre[int(len(normals) * 0.8) : :]
        + dos_pre[int(len(dos) * 0.8) : :]
        + portscan_pre[int(len(portscan) * 0.8) : :]
    ]
    external_test += [
        x[1:-1]
        for x in bot_pre[int(len(bot) * 0.8) : :]
        + inf_pre[int(len(inf) * 0.8) : :]
        + bruteforce_pre[int(len(bruteforce) * 0.8) : :]
        + sqlinj_pre[int(len(sqlinj) * 0.8) : :]
        + xss_pre[int(len(xss) * 0.8) : :]
    ]
    external_test += [
        x[1:-1]
        for x in ftp_pre[int(len(ftp) * 0.8) : :]
        + ssh_pre[int(len(ssh) * 0.8) : :]
        + hulk_pre[int(len(hulk) * 0.8) : :]
        + goldeneye_pre[int(len(goldeneye) * 0.8) : :]
        + slowloris_pre[int(len(slowloris) * 0.8) : :]
        + slowhttp_pre[int(len(slowhttp) * 0.8) : :]
    ]
